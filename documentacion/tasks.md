...
- **✅ Completado (2026-08-25): el motor ya escribe `isHome`.** El algoritmo de round robin **ya distinguía** `home` de `away` al armar cada cruce; lo que faltaba era persistir esa distinción, que se perdía al guardar. Cubierto por `src/modules/competitions/engine.factory.spec.ts` (**9 tests**) y verificado en vivo: un fixture de 3 competidores deja 3 partidos y 6 resultados, **ninguno con `is_home` en NULL**, y cada cruce con exactamente un local y un visitante.
- **🐛 Bug encontrado al completarlo, y peor que el original: el fixture de una disciplina individual estaba roto.** El motor decidía si los ids eran de equipos o de participantes con `homeId.includes('-')`, comentado como *"uuid check simple"* — pero **los uuid de participante también tienen guiones**, así que la rama individual era código muerto y se escribía un `teamId` apuntando a un participante. **Reproducido en vivo contra Postgres antes de arreglarlo: `POST /competitions/:id/fixture` con `participantIds` devolvía 500** por violación de foreign key. Ahora el tipo de competidor lo declara el service, que es el único que sabe si el DTO trajo `teamIds` o `participantIds`. Verificado en vivo por los dos caminos: individual → 6 resultados con `participant_id` y ningún `team_id`; por equipos → 6 con `team_id` y ningún `participant_id`.
- **🐛 Y un tercero, que destapó el test:** el motor hacía `ids.push('BYE')` **sobre el arreglo del llamador**, o sea sobre el `teamIds` del DTO. Escribir los tests con una constante compartida hizo visible la mutación. Ahora trabaja sobre una copia.
- **⚠️ Anotado, no arreglado:** `generateFixture` **no es transaccional**. Se vio al reproducir el 500: el `match.create` ya había pasado, así que quedó un partido huérfano sin resultados y el reintento devolvía 409 *"La competencia ya tiene un fixture generado"*. Es la misma clase de problema que R13 y merece su propia tarea.
- **Contraprueba:** restaurando la heurística vieja fallan **5 de los 9** tests.
