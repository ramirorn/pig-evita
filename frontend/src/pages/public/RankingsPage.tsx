  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12 animate-fade-in">
        ...
        <h1 className="text-3xl font-bold text-primary-800 mb-4">Rankings y Resultados</h1>
        <p className="text-primary-600 max-w-2xl mx-auto text-lg">
          Tablas de posiciones, fixtures y resultados actualizados de todas las competencias activas.
        </p>
      </div>
      ...
      <div className="max-w-xl mx-auto mb-10 relative">
        ...
        <Input 
          className="pl-12 h-12 text-lg rounded-2xl border-primary-200 focus-visible:ring-primary-500"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCompetitions?.map((competition) => (
            <Link key={competition.id} to={`/competencias/${competition.id}`}>
              <div className="card p-6 h-full hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col justify-between border-t-4 border-t-primary-500">
                <div>
                  ...
                  <div className="space-y-1 mt-4 text-sm">
                    <div className="flex justify-between text-primary-600">
                      ...
                      <span className="font-medium text-primary-800">{competition.discipline?.name}</span>
                    </div>
                    <div className="flex justify-between text-primary-600">
                      <span>Categoría:</span>
                      <span className="font-medium text-primary-800">{competition.category?.name}</span>
                    </div>
                    <div className="flex justify-between text-primary-600">
                      <span>Etapa:</span>
                      <span className="font-medium capitalize text-primary-800">{competition.stage}</span>
                    </div>
                  </div>
                </div>
                <div className="mt-6 pt-4 border-t border-primary-100 flex items-center justify-between text-primary-600 group-hover:text-primary-800 font-medium">
                  <span className="flex items-center gap-2">
                    <CalendarDays className="w-4 h-4" /> Ver Fixture
                  </span>
                  <span className="flex items-center gap-2">
                    <Medal className="w-4 h-4" /> Posiciones
                  </span>
                </div>
              </div>
            </Link>
